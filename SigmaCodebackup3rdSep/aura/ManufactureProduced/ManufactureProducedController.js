({
    ...
    saveStock: function (component, event, helper) {
       ...
       /* if(component.get("v.manfObj.sigmaerpdev2__Produced_Quantity__c") != qcPassedQuantity){
        //alert(component.get("v.manfObj.sigmaerpdev2__Moved_Quantity__c"));
         //if(component.get("v.manfObj.sigmaerpdev2__Moved_Quantity__c") != qcPassedQuantity){   
            var toastEvent = $A.get("e.force:showToast");
            toastEvent.setParams({
                "type": "error",
                "title": "Error!",
        ...
        action.setCallback(this, function (response) {
            ...
            // alert('DUPLICATE_VALUE>>>'+state);
            if (state === "SUCCESS") {
                //added newly on 16/2/2021 to handle issue Bug-6030
				//alert('state'+state);
                if (response.getReturnValue() != 'passed') {
                     //alert('response.getReturnValue()'+response.getReturnValue());
                    var str = response.getReturnValue();
                    if(str.includes("DUPLICATE_VALUE")){   
                      //  alert('inside duplicate error');
                        var toastEvent = $A.get("e.force:showToast");
                        toastEvent.setParams({
                            "type": "error",
                            "title": "Error!",
                            "message": "Duplicate Serial Number value found in csv file."
                        });
                        toastEvent.fire();
                        component.set("v.SMData.serialNumberLst.length", 0);
                    }
                     else 
                    {
                       // alert('inside else get lost'+response.getReturnValue() );
            ...
            action.setCallback(this, function (response) {
                ...
                if (state == "SUCCESS") {
                   ...
                   /* var qcPassedQty = component.get("v.SMData.manfObject.sigmaerpdev2__QC_Passed_Qty__c");                                        
                    ...
                    component.set("v.dataobjNewDmgdLocId", response.getReturnValue().defDamagedLocObj.Id);                    
                    //ends here
    ...
    closeModal : function(component, event, helper){
     ...
     clearNumbers : function(component, event, helper){
		...
		//var length = component.get("v.manfObj.sigmaerpdev2__Produced_Quantity__c");  
		 //added below line on 14/12/2020 to fix 5827 issue       
		var length = component.get("v.passedQty");             
        var serialList = [];        
  ...
  /*  addSerialNumber : function(component, event, helper){         
            ...
            try{
                ...
                reader.onload = function(){             
                   ...
                   // var prodFinCount = component.get("v.manfObj.sigmaerpdev2__Produced_Quantity__c");
                    var prodFinCount = component.get("v.passedQty");
     ...
     addSerialNumber : function(component, event, helper){         
        ...
        if(fileInput == null){			            
            //manual entry flow
            var srlNo = component.get('v.serials');
           // alert('srlNo.length==='+srlNo.length);
            var myArray = [];
            for(var i = 0; i < srlNo.length; i++){
                myArray.push(JSON.stringify(srlNo[i].SerialNo));
              //  alert(myArray);
            ...
            component.set("v.SMData.serialNumberLst", finArr); 
           // alert('serialNumberLst>>>>'+ JSON.stringify(component.set("v.SMData.serialNumberLst")));
            //added newly on 9/5/2020 to disable produced qty after serial number is generated
            component.set("v.disableAfterSerNum", true);
            //ends here
           ...
           // alert('upload');
            ...
            try{
                ...
                reader.onload = function(){             
                    ...
                    for(var i=1;i<rows.length;i++){
                       var cells = rows[i].split(',');               
                        if(cells[selRow] != '' || cells[selRow] != undefined){                           
                            ...
                            //alert('cells[selRow]>>>'+cells[selRow]);
                           // alert('finalSerList>>>'+finalSerList);
                        }
                    }                     
                    finalSerList.pop();                     
                    for(var i=0;i<finalSerList.length;i++){                    	
                    ...
                    //added newly on 30-11-2020 to provide product validation in csv file
                                  
                    var selProdName = component.get("v.manfObj.sigmaerpdev2__Product__r.Name")
                 //   alert('selProdName>'+selProdName);
                    var myArray2 = rows[0].split(',');                         
                    var srchString2;
                    var selRow2;
                    var misMatch = false;
                    for(var i=0;i<myArray2.length;i++){
                        srchString2 = myArray2[i].toLowerCase();               
                        var iVal = srchString2.indexOf("product");               
                        if(iVal == 0){
                            selRow2 = i;                    
                            break;
                        }                   
                    ...
                    for(var i=1;i<rows.length;i++){
                       var cells = rows[i].split(',');               
                        if(cells[selRow2] != '' || cells[selRow2] != undefined){                           
                       		finalSerList2.push(cells[selRow2]);
                        }
                    }                     
                    finalSerList2.pop();                     
                    for(var i=0;i<finalSerList2.length;i++){                    	
                        if(finalSerList2[i] != undefined){
                        	finalSerList2[i] = finalSerList2[i].replace(/(\r\n|\n|\r)/gm,"");
                            finalSerList2[i] = finalSerList2[i].replace (/("\"")/gm,"");
                        }                     		
                    } 
                 
                    for(var i=0;i<finalSerList2.length;i++){
                        //alert(finalSerList2[i]);
                        var indv = finalSerList2[i].split(',');                        
                        if(indv != selProdName){
                            misMatch = true;
                        }
                    }
                   // alert(misMatch);
                    if(misMatch){
                        ...
                        toastEvent.setParams({
                            "type":"Error",
                            "title": "Error!",
                            "message": "Produced product of MR doesn't match with product listed in csv file.Check and try again."
                        });
                        toastEvent.fire();
                        return;
                    }
                    ...
                    var prodFinCount = component.get("v.manfObj.sigmaerpdev2__Produced_Quantity__c");
                    //alert(prodFinCount);
                    var pQty = component.get("v.passedQty");
                 //   alert(pQty);
                   // alert(finalSerList.length);
                    //return;
                    //if(prodFinCount != finalSerList.length){    
                    if(pQty != finalSerList.length){            
                        var toastEvent = $A.get("e.force:showToast");
                        toastEvent.setParams({
                            "type":"Error",
                            "title": "Error!",
                            //"message": "Serial codes count doesn't match Produced Quantity count.Upload failed."
                            "message": "Serial codes count doesn't match passed quantity count.Upload failed."
                        });
                        toastEvent.fire();
                        return;
                    }

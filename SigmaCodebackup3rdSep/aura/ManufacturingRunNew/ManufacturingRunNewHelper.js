({
    ...
    saveManuRunH : function(cmp, event, helper) {
        ...
        //alert('ManufacRunObj=='+ManufacRunObj);
       // alert('fullDataList=='+fullDataList);
    ...
    AutoReserveStock : function(component, event, helper){
        ...
        if(skipAllocationProcess == true){
            ...
            //new code to show warning msg if resources have not yet been added to MR            
            var action84 = component.get("c.chkResourcesAdded"); 
            action84.setParams({					
                "mrID": component.get("v.ManufacRunObj.Id")
            });
            action84.setCallback(this, function (response84) {
            	var response = response84.getReturnValue();
                if(response == false){
                	var toastEvent = $A.get("e.force:showToast");
                    toastEvent.setParams({
                        "type":"warning",
                        "title": "Warning!",
                        "message": "No resources have been allocated for this Manufacturing Run Process !!!"
                    });
                    toastEvent.fire();    
                }
                ...
                action2.setCallback(this, function(response1){
                    ...
                    if(state == 'true'){ 
                        ...
                        //commented below section on 3/4/2020
                        /*if(skipAllocationProcess == false){                	
                            component.set('v.status','Stock Reserved');
                            component.set('v.ManufacRunObj.sigmaerpdev2__Status__c','Stock Reserved');
                        }else{                    
                            component.set('v.status','Produced');
                            component.set('v.ManufacRunObj.sigmaerpdev2__Status__c','Produced'); 
                            //commented below line on 21/8/2019 because Produced quantity was coming same as required qty during add flow
                            //component.set('v.ManufacRunObj.sigmaerpdev2__Produced_Quantity__c',component.get('v.ManufacRunObj.sigmaerpdev2__Required_Quantity__c'));                                         
    ...
    editManufRunH : function(cmp, event, helper) {                	
        ...
        action.setCallback(this, $A.getCallback(function (response) {
            ...
            if (state === "SUCCESS") {
               ...
               // alert( response.getReturnValue().manfRun.sigmaerpdev2__Recipe__r.Name)
                //alert( response.getReturnValue().manfRun.sigmaerpdev2__manufactuting_Run_Plant_name__r.Name)
                ...
                cmp.set("v.ManufacRunObj.sigmaerpdev2__Production_Manager__c", response.getReturnValue().manfRun.sigmaerpdev2__Production_Manager__c);
                cmp.set("v.ProductionManager", response.getReturnValue().manfRun.sigmaerpdev2__Production_Manager__r.Name);
                 cmp.set("v.ManufacRunObj.sigmaerpdev2__manufactuting_Run_Plant_name__c", response.getReturnValue().manfRun.sigmaerpdev2__manufactuting_Run_Plant_name__c);
                cmp.set("v.ManufacturingRunPlantName", response.getReturnValue().manfRun.sigmaerpdev2__manufactuting_Run_Plant_name__r.Name);
               // alert(response.getReturnValue().manfRun.sigmaerpdev2__Production_Manager__r.Name)
        ...
        ResourceAction.setCallback(this, function (response) {
            ...
            if(state == 'SUCCESS'){
              ...
              //  alert('state>>>'+state);
				...
				var errors = response.getError();
                console.error(errors);
                alert('error while getting resources:'+JSON.stringify(errors));
                return;
            }                            
        });

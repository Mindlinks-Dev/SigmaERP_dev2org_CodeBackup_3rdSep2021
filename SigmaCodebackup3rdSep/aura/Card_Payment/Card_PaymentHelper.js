({
	...
	makeChargeAfterValidation : function(component,event)
	{
        ...
        // alert('receivedFrom>>>>>>>'+receivedFrom);
        var accountSFId = component.get('v.recordId');
        // alert('accountSFId>>>>>>>'+accountSFId);
       ...
       //alert('paidAmount>>>>>>>'+paidAmount);
        ...
        var action = component.get("c.fetchPaymentVendor");  
        action.setParams({
            "custId"  : accountSFId
        });
        ...
        action.setCallback(this, function(response) 
                           {           
                               ...
                               //alert('state22222'+state);
                               if (component.isValid() && state == "SUCCESS")
                               {               
                                   vendorType = response.getReturnValue();
                                   var returnValues = response.getReturnValue(); 
                                  // alert('Card payment helper returnValues>>>>>>'+returnValues);
        ...
        setTimeout(function()
                   {
                       if(vendorType == 'Stripe')
                       {
                          doPayPalPayment();
                       }
                       else if(vendorType == 'Paypal')
                       {
                           doPayPalPayment();
                       }        
                   }, 1000);        
        ...
        {
              var cardsub = component.get("v.subpayment");
            ...
            if(cardsub==true)
            {
                payment.sigmaerpdev2__Account_Subscription_Interval__c = orderSFId;
            }else
            {
             payment.sigmaerpdev2__Sigma_Order__c = orderSFId;
            }
          ...
          /// alert('cardsub'+paidAmount);
             if(cardsub==true)
            {
              var actionPaypal = component.get("c.stripePaymentsubscription"); 
            ...
            else
            {
             // alert('else of payment'); 
               ...
               var regid =   component.get("v.regid"); 
                var appointmentId=  component.get("v.MainWrapperObject.AppointmentId"); 
            actionPaypal.setParams
            ({
                "payment"  : payment,
            ...
            actionPaypal.setCallback(this, function(response) 
                                     {
                                         ...
                                         {               
                                            ...
                                            if ( returnValues == null || returnValues == '')
                                             ...
                                             alert('Payment done successfully');
                                             component.set("v.paymentdone",true);
                                              document.getElementById("Accspinner").style.display = "none";
                                           //window.location.href = 'http://staging.quicksolar.com/payment/payment_controller/handlePaymentResponse?responseCode=0&responseToken=rA6OiCYKeY&transactionId=ch_1HbIz';
                                             // window.open('http://staging.quicksolar.com/payment/payment_controller/handlePaymentResponse?responseCode=0&responseToken=rA6OiCYKeY&transactionId=ch_1HbIz');
                                              var transSuccessEvent = component.getEvent("transSuccessEvent");   
               								transSuccessEvent.fire();
                                         }
                                         else if (state == "ERROR") 

({
	expandOrHide : function(component, event, helper) {
		var rowId = component.get("v.sigmaOrderIlpliWrapper.orderLineWrap.Id");		  
    ...
    autoSelectQnty : function(component, event, helper){        
        var orderedqty = component.get("v.sigmaOrderIlpliWrapper.orderLineWrap.sigmaerpdev__Net_Quantity__c");        
        var tempWrap = component.get("v.sigmaOrderIlpliWrapper.wrapperIlpli");
        var prodAttrType = component.get("v.sigmaOrderIlpliWrapper.orderLineWrap.sigmaerpdev__Product__r.sigmaerpdev__Attribute_Type__c");        
        
        if(prodAttrType == 'BULK'){                       
            if(tempWrap[0].AvailableQuantity >= orderedqty){
            	tempWrap[0].EnterQuantity = orderedqty;
            	component.set("v.sigmaOrderIlpliWrapper.wrapperIlpli",tempWrap);     
            }else{
        ...
        console.log('list==='+JSON.stringify(component.get("v.sigmaOrderIlpliWrapper")));
    }
})
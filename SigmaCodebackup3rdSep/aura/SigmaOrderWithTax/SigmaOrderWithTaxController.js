({
     handleLookupValueselected: function (component, event, helper) {
        ...
        if(event.getParam("objectAPIName")==='sigmaerpdev2__Tax_Treatment__c'){
   ...
   NoPackageShipment : function(cmp,event,helper)
    ...
    getAccountData: function (component, event, helper) {
        ...
        if (component.get("v.recordName"))
        {
          // alert('Inside true');
            ...
            helper.helperGetAccountDataforcontact(component,event,helper,component.get("v.sigmaOrder.sigmaerpdev2__AccountId__c")); 
        }
    },
    ...
    AddOrderLine: function (component, event, helper) {
       //alert('inside AddOrderLine'+JSON.stringify(component.get("v.orderLinesData")))
        var d = new Date();
        ...
        if(!component.get('v.sigmaOrder.sigmaerpdev2__Tax_Treatment__c')){
            component.set("v.errorMessage", "Select the Tax Treatment before adding Order Lines.");
            component.set("v.isError", true);
            window.scrollTo(0, document.getElementById("HeaderErrorDiv").offsetTop);
            return;
        }  
         if(!component.get('v.sigmaOrder.sigmaerpdev2__BillingPersonNew__c')){
            component.set("v.errorMessage", "Select the Billing Person before adding Order Lines.");
            component.set("v.isError", true);
            window.scrollTo(0, document.getElementById("HeaderErrorDiv").offsetTop);
            return;
        }  
    ...
    // alert('addorderline'+JSON.stringify(orderLinesData));
        /*var idListStr;
        ...
        orderLines.push({
           ...
           //'TaxCodeName': taxTreatmentData.Name,
            'orderLines': {
              ...
              // 'sigmaerpdev2__Tax_Code__c' :taxTreatmentData.Id
    ...
    handleComponentEvent: function (component, event, helper) {
        ...
        else if (event.getParam("flag") == 'taxCalculation') {
           // alert('2')
   ...
   handleDiscountChange: function (component, event, helper) {
       ...
       /* var olRecord = component.get("v.orderLinesData");
        ...
        component.set("v.orderLinesData", olRecord);*/
      if(component.get("v.sigmaOrder.sigmaerpdev2__Discount__c")<0)
        {
            alert('Discount should not be negative');
            component.set("v.sigmaOrder.sigmaerpdev2__Discount__c",component.get("v.sigmaOrder.sigmaerpdev2__Discount__c")*-1);
        }
        else if(component.get("v.sigmaOrder.sigmaerpdev2__Discount__c")>100)
        {
            alert('Discount should not be more that 100%.');
            component.set("v.sigmaOrder.sigmaerpdev2__Discount__c",100);
        }
        ...
        if (!allValid) {
            ...
            switch (true) {
                case (!so.sigmaerpdev2__AccountId__c):
                    ...
                    window.scrollTo(0, document.getElementById("HeaderErrorDiv").offsetTop);
                    return;
                     case (!so.sigmaerpdev2__Tax_Treatment__c):
        ...
        if (validationFlag) 
        {
           ...
           // alert('olData>>'+JSON.stringify(component.get("v.orderLinesData")));
            ...
            for (var i = 0; i < olData.length; i++) {
               ...
               //if (!(olData[i].orderLines.sigmaerpdev2__Product__c && olData[i].orderLines.sigmaerpdev2__Delivary_Date__c && olData[i].orderLines.sigmaerpdev2__Order_Status__c && olData[i].orderLines.sigmaerpdev2__Tax_Code__c )) {
                ...
                if (!olData[i].orderLines.sigmaerpdev2__Tax_Code__c && !olData[i].orderLines.sigmaerpdev2__Back_Order__c
                        && !olData[i].orderLines.sigmaerpdev2__Splited_Order_Line__c )
        ...
        /*if(tempTotalOrderedQtyForValidation>parseInt(splitBackOrderLines.parentQty))
        {
          ...
          for(var i=0;i<splitBackOrderLines.splitList.length;i++)
          {
              ...
              if(splitBackOrderLines.splitList[i].qty)
              {
                  ...
                  {
                  console.log('index>>>>>'+JSON.stringify(orderLinesData));
                        console.log('splitlist>>>>>'+JSON.stringify(splitBackOrderLines.splitList[i]));
                      orderLinesData.push({
                          'hasBackOrder': false,
                          'isDisabled': true,
                          'isDisplay': true,
                          'productName': orderLinesData[ind].productName,
                         // 'TaxCodeName':orderLinesData[ind].TaxCodeName,
                          ...
                          'orderLines': {
                             ...
                             /* 'sigmaerpdev2__Tax_Code__c':orderLinesData[ind].orderLines.sigmaerpdev2__Tax_Code__c,
                              'sigmaerpdev2__Tax_Rate__c':orderLinesData[ind].orderLines.sigmaerpdev2__Tax_Rate__c,
                              'sigmaerpdev2__tax_amount__c':orderLinesData[ind].orderLines.sigmaerpdev2__tax_amount__c,*/
                  ...
                  //tempIndex++;
                /*var total=(splitBackOrderLines.splitList[i].qty+splitBackOrderLines.parentQty)*orderLinesData[ind].orderLines.sigmaerpdev2__Total_Amount__c;
                  alert('total>>>>>'+total)*/
          ...
          {         
             ...
             var totalprice=(orderLinesData[ind].orderLines.sigmaerpdev2__Quantity__c * orderLinesData[ind].orderLines.sigmaerpdev2__Total_Amount__c);
          ...
          else
          {
             // orderLinesData[ind].orderLines.sigmaerpdev2__Total_Price__c =orderLinesData[ind].orderLines.sigmaerpdev2__Net_Quantity__c * orderLinesData[ind].orderLines.sigmaerpdev2__Total_Amount__c;
              orderLinesData[ind].orderLines.sigmaerpdev2__Total_Price__c =orderLinesData[ind].orderLines.sigmaerpdev2__Quantity__c * orderLinesData[ind].orderLines.sigmaerpdev2__Total_Amount__c;
          }

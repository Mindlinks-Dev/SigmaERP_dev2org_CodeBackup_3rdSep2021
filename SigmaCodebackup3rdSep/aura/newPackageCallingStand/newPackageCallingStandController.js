({
	doInit : function(component, event, helper) {
        ...
        if(state === "SUCCESS"){
            var orderresult = a.getReturnValue();{
                if(orderresult.sigmaerpdev__Sigma_order__c){
                    alert("Order is not checked in Custom Settings, Please select Order");
    ...
    $A.enqueueAction(orderaction);
        ...
        var actionDetails = component.get("c.getstandorderdetails");
         ...
         actionDetails.setCallback( this, function(a){ 
              ...
              if (state === "SUCCESS")
                    {                
                        ...
                        {
                         	component.set("v.recordName",sigmaOrder.Account.Name);
                         	component.set("v.record1",sigmaOrder.OrderNumber);
                         	component.set("v.package.sigmaerpdev__Customer__c",sigmaOrder.Account.Id); //commented by chandana
                        	component.set("v.customername",sigmaOrder.Account.Name);
	 ...
	 $A.enqueueAction(actionDetails);
      ...
      var action17 = component.get("c.getstandpackageproddetails");
        action17.setParams
        ({ 
            "Id": sid            
        });
